...
export default function Sidebar() {
  ...
  return (
    <aside className="fixed left-0 top-0 z-30 h-screen w-[220px] border-r border-[var(--line)] bg-[var(--card)]/95 px-4 py-6 shadow-[10px_0_40px_rgba(15,23,42,0.03)] backdrop-blur-xl max-lg:hidden">
      <div className="mb-9 flex items-center gap-3 px-1">
        ...
        <h1 className="text-[21px] font-semibold tracking-[-0.06em] text-[var(--text)]">qanot</h1>
      </div>
        ...
        {links.map((item) => {
          ...
          return (
            <Link
              key={item.href}
              href={item.href}
              className={`flex h-11 items-center gap-3 rounded-[16px] px-3.5 text-[14px] transition ${active ? "bg-[var(--blue-soft)] text-[var(--blue)]" : "text-[var(--muted)] hover:bg-[var(--hover)] hover:text-[var(--text)]"}`}
            >
