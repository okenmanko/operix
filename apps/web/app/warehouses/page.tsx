...
export default function WarehousesPage() {
  ...
  const totalValue = warehouses.reduce((sum, item) => sum + Number(item.totalValue || 0), 0);

  return (
    <AppLayout title="Omborlar" subtitle="Skladlar ichidagi mahsulotlar va jami qoldiq.">
      ...
      <div className="premium-card mb-5 p-6">
        ...
        <div className="mt-5 grid grid-cols-[1fr_1fr_140px] gap-4">
          ...
          <table className="w-full text-left text-[14px]">
            <thead className="bg-[#f8fafc] text-[11px] uppercase tracking-[0.14em] text-[#8aa0ba]">
              <tr>
                ...
                <th className="p-4 font-normal">Jami summa</th>
              </tr>
            </thead>
            <tbody>
              {warehouses.map((warehouse) => (
                <tr key={warehouse.id} className="border-t border-[#edf2f7] transition hover:bg-[#f8fafc]">
                  <td className="p-4 text-[#111827]">
                    <Link href={`/warehouses/${encodeURIComponent(warehouse.id)}`} className="hover:text-[#315efb]">
                      {warehouse.name}
                    </Link>
                  </td>
                  ...
                  <td className="p-4 text-[#111827]">{money(warehouse.totalValue || 0, "USD")}</td>
                </tr>
              ))}
              ...
              {!warehouses.length ? (
                <tr>
                  <td colSpan={5} className="p-10 text-center text-[#8aa0ba]">
                    Ombor yo‘q. Integratsiyalar sahifasidan MoySklad sync qiling.
                  </td>
                </tr>
              ) : null}
            </tbody>
